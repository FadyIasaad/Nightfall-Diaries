...
def make_frame(video_id, scene_index, scene, title, image_path, total_scenes):
    ...
    top_overlay = Image.new("RGBA", (WIDTH, 230), (0, 0, 0, 105))
    bg.alpha_composite(top_overlay, (0, 0))

    subtitle_h = 520
    subtitle_y = HEIGHT - subtitle_h - 70
...
def create_video(video_id, title, scene_payload):
    ...
    for i, scene in enumerate(scenes, start=1):
        ...
        duration = max(3.0, audio_clip.duration + 0.45)
...
def main():
    ...
    log(
        ...
        video_id,
        "GENERATE_VIDEO",
        f"Created improved voice full-subtitle video: {video_path}. Voice: {voice_source}",
    )
